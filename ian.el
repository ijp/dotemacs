...
;;; Greek letter map
(defmacro definsertmap (name &rest pairs)
  `(progn (setq ,name (make-sparse-keymap))
         ,@(mapcar (lambda (pair)
                     `(define-key ,name (vector ,(car pair))
                        (lambda (count)
                          (interactive "P")
...
(define-minor-mode greek-letter-mode
  "Replaces current keymap with one that inserts greek letters"
  nil
  "Grk"
  greek-keymap)
...
(defvar hypermod
  (get 'hyper 'modifier-value))
(defun hyperize (char)
  (+ hypermod char))

(setq greek-hyper-keymap
  (cons 'keymap
        (mapcar (lambda (x) (cons (hyperize (car x)) (cdr x)))
                (cdr greek-keymap))))

(define-minor-mode greek-hyper-mode
  "Replaces current keymap with one that inserts greek letters"
  :init-value nil
  :keymap greek-hyper-keymap
  :global t)

;; Bad Solution for my problem -- i.e doesn't work
;; (define-prefix-command 'hypers)
;; (setcdr hypers (cdr greek-keymap))
;; (global-set-key (kbd "C-c z") 'hypers)
;; (global-set-key (kbd "<hyper>") 'hypers)


;; Solution by mickeynp on twitter
;; http://paste.lisp.org/display/120016

;; (defvar ctl-h-map (make-sparse-keymap)
;;   "Hyper mirror-map of `ctl-x-map'")

;; (defun convert-key-to-hyper (event-type command)
;;   (when (not (event-modifiers event-type))
;;       (global-set-key
;;         (vector (event-apply-modifier event-type 'hyper 24 "H-")) command)))
